...
- [ ] **DATA-04**: `phase-1/PHASE_1_FORMAT_CONTRACT.md` 기준으로 입력 데이터와 후보 데이터 스키마 검증
- [ ] **DATA-05**: 승인 결과와 PO 초안 이력을 CSV/JSON으로 저장

### Monitor Agent

- [ ] **MON-01**: Mock 재고 스냅샷을 주기적으로 스캔하여 현재고 < 안전재고 조건 감지
- [ ] **MON-02**: 결품 조건 충족 시 `shortage_event` 포맷으로 이벤트 발생
