...
- [x] 자재 마스터 데이터 구조 정의
- [x] 재고 스냅샷 데이터 구조 정의
- [x] 웹 검색 후보 캐시의 최소 필드 정의
- [x] `현재고 < 안전재고` 조건 감지 로직 구현
- [x] 부족 수량 계산
- [x] 다음 단계로 넘길 `shortage_event` 생성
- [x] 검색 쿼리 생성을 위한 핵심 스펙 키워드 추출 준비
...
- [x] 재고 부족 자재가 1개 이상 정상 감지된다.
- [x] 부족 수량이 정확히 계산된다.
- [x] `shortage_event`가 Phase 2에서 바로 사용할 수 있는 형태로 생성된다.
- [x] [Phase 1 Format Contract](PHASE_1_FORMAT_CONTRACT.md)의 필수 필드가 모두 채워진다.
- [x] mock 데이터만으로 단독 실행이 가능하다.
- [x] 필드명 변경 사항이 생기면 `REQUIREMENTS.md`와 팀원에게 공유된다.
